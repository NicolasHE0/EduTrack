...
function Asistencia({materias,asistencia:asistenciaRaw,asistenciaMateria:asmRaw,upd,tema:t}) {
  ...
  const calcTotal = (arr) => arr.reduce((acc,a) => {
    ...
    // Compatibilidad: registros viejos con campo media (boolean)
        ...
        <div className="g2">
          <div className="card" style={{height:"fit-content"}}>
            ...
            <div style={{marginBottom:8}}>
              ...
              <input className="inp" type="number" min="0.1" max="1" step="0.1"
                value={formMat.valor||"1"}
                onChange={e=>setFormMat(f=>({...f,valor:e.target.value}))}
                placeholder="Ej: 0.2"/>
            </div>
            <div style={{marginBottom:12}}><div className="lbl">Observaciones</div>
...
function Informes({materias,calificaciones:calsRaw,trimestres:triRaw,config,promedioMat,promedioGeneral,colMat,nomMat,tema:t}) {
  ...
  const generarPDF = () => {
    ...
    const pg = tipo==="anual"
      ...
      : (ps.length ? Math.ceil(ps.reduce((a,b)=>a+b,0)/ps.length*2)/2 : null);
    const promedioFinalHTML = pg
      ? `<div style="margin-top:28px;padding:16px 20px;background:#EFF6FF;border:1.5px solid #BFDBFE;border-radius:12px;display:flex;justify-content:space-between;align-items:center">
          ...
          <div style="font-size:32px;font-weight:800;font-family:monospace;color:#1D4ED8">${pg.toFixed(2)}</div>
        </div>` : "";
          ...
          {(()=>{
            ...
            const pg=ps.length
              ? tipo==="anual"
                ...
                : Math.ceil(ps.reduce((a,b)=>a+b,0)/ps.length*2)/2
              : null;
            return pg?(
